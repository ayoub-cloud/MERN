...
const multiTypePokémon = pokémon.filter(p => p.types.length > 1);
...
const firstTypeWithSecondFlying = pokémon.filter(p => p.types.length > 1 && p.types[1] === "flying").map(p => p.types[0]);
